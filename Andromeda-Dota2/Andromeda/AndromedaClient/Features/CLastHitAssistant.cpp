...
namespace
{
	...
	auto LooksLikeLaneCreepByName( C_BaseEntity* entity ) -> bool
	{
		...
		return EntityTextContains( entity , "npc_dota_creep_" ) ||
	...
	{
		const float configured = std::isfinite( Settings::LastHitAssistant::DetectRange ) ?
			Settings::LastHitAssistant::DetectRange : 950.f;
		...
		LocalHeroInfo& out , bool requireHeroIdentity = false ) -> bool
	{
		...
		const int rawDamageMax = offsets.hasDamageMax ? ReadField<int>( hero , offsets.damageMax , rawDamageMin ) :
			rawDamageMin;
		...
		int localPlayerId , LocalHeroInfo& out , int& cachedHeroIndex ) -> bool
	{
		...
		for ( int chunkIndex = 0; chunkIndex < MAX_ENTITY_LISTS; ++chunkIndex )
		{
			...
			for ( int entryIndex = 0; entryIndex < MAX_ENTITIES_IN_LIST; ++entryIndex )
			{
				...
				const int controllerPlayerId = canReadControllerPlayer ?
					ReadField<int>( entity , offsets.controllerPlayerId , -1 ) : -1;
		...
		LocalHeroInfo& out ) -> bool
	{
		...
		const uint8_t preferredTeam = IsPlayableTeam( cachedLocalTeam ) ? cachedLocalTeam : 0;
		...
		if ( IsPlayableTeam( preferredTeam ) &&
			ResolveHeroNearScreenCenter( entitySystem , offsets , out , cachedHeroIndex ,
				hasLocalPlayerId ? localPlayerId : -1 , preferredTeam ) )
		{
			if ( out.team == preferredTeam )
			{
				cachedHeroFromScreenCenter = true;
				cachedLocalTeam = preferredTeam;
				return true;
			}
		}
		...
		int health , int maxHealth , uint8_t team ) -> bool
	{
		...
		if ( LooksLikeLaneCreepByName( entity ) )
			return !IsPlayerControlledUnit( entity , offsets );
		if ( maxHealth < 80 || maxHealth > 1000 )
			return false;
		return !IsPlayerControlledUnit( entity , offsets );
	}
		...
		const LastHitScanStats& stats , const std::vector<EnemyCreepSnapshot>& enemyCreeps ) -> void
	{
		...
		const float timingBuffer = (std::max)( std::clamp( Settings::LastHitAssistant::HealthBuffer , 0.f , 250.f ) ,
			kMinimumVisualLeadHp );
		const float starThreshold = hero.entity ? static_cast<float>( hero.attackDamage ) + timingBuffer : 0.f;
	...
	auto CollectCandidatesFresh( LocalHeroInfo& heroOut ) -> std::vector<CreepCandidate>
	{
		...
		const float timingBuffer = (std::max)( std::clamp( Settings::LastHitAssistant::HealthBuffer , 0.f , 250.f ) ,
			kMinimumVisualLeadHp );
		const float lethalHealth = static_cast<float>( hero.attackDamage ) + timingBuffer;
		for ( int chunkIndex = 0; chunkIndex < MAX_ENTITY_LISTS; ++chunkIndex )
		{
			auto* chunk = entitySystem->m_pIdentityChunks[chunkIndex];
			if ( !chunk )
		...
		std::sort( candidates.begin() , candidates.end() , []( const CreepCandidate& left ,
			const CreepCandidate& right )
		{
		...
		std::sort( enemyCreeps.begin() , enemyCreeps.end() , []( const EnemyCreepSnapshot& left ,
			const EnemyCreepSnapshot& right )
		{
